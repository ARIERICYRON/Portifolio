...
This builds the app and pushes `/build` to the `gh-pages` branch. The custom domain is configured via `public/CNAME` (copied into every build automatically).
...
3. Set `REACT_APP_CHAT_API_URL` in `.env.production` (already done — see that file) to your Vercel deployment's URL, then rebuild/redeploy the GitHub Pages site.
